    ...
    private class ScoreToSwitchAutoCG extends CommandGroup {
        ScoreToSwitchAutoCG(String fieldData) {
            if (position == fieldData.charAt(0)) {
                addSequential(new AutoStraightDrive(0.7, AutonMath.inchesToEncoderTicks(77)));
                addSequential(new AutoGyroRotate(90 * turnMult));
                addSequential(new AutoStraightDrive(0.6, AutonMath.inchesToEncoderTicks(12)));
            } else {
                addSequential(new AutoStraightDrive(0.7, AutonMath.inchesToEncoderTicks(168)));
                addSequential(new AutoGyroRotate(90 * turnMult));
            }
        }
    }
